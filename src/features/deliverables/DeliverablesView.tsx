...
import { matchesDeliverableFilters, type DeliverableFilters } from './deliverableFilters';
import {
  abSlopeForAngle,
  clampNumber,
  clipPolygonForSplit,
...
export function DeliverablesView({
  ...
  const [filters, setFilters] = React.useState<DeliverableFilters>({
  ...
  const filteredTasks = React.useMemo(
    () => tasks.filter((task) => matchesDeliverableFilters(task, filters, currentUser)),
      ...
      <section className="filters" aria-label="Deliverable filters">
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value as DeliverableFilters['status'] }))}
          >
        ...
        <label>
          ...
          <select
            value={filters.phase}
            onChange={(event) => setFilters((current) => ({ ...current, phase: event.target.value as DeliverableFilters['phase'] }))}
          >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => setFilters((current) => ({ ...current, priority: event.target.value as DeliverableFilters['priority'] }))}
          >
